...
public:
    ...
    [[nodiscard]] std::vector<T> Compute(const std::vector<T>& host_vector_a,
        ...
        RunCudaKernel(kNumBlocks, kBlocksize);
...
private:
    ...
    void RunCudaKernel(int32_t num_blocks, int32_t block_size) {
        CallKernel<T>(num_blocks, block_size, device_vector_a_.Get(),
                      device_vector_b_.Get(), device_result_.Get(),
                      vector_size_);
    }

    template <typename U>
    typename std::enable_if<std::is_same<U, float>::value>::type CallKernel(
        int32_t num_blocks, int32_t block_size, U* d_a, U* d_b, U* d_result,
        int32_t size) {
        VectorPerElemMinFloat<<<num_blocks, block_size>>>(d_a, d_b, d_result,
                                                          size);
    }

    template <typename U>
    typename std::enable_if<std::is_same<U, double>::value>::type CallKernel(
        int32_t num_blocks, int32_t block_size, U* d_a, U* d_b, U* d_result,
        int32_t size) {
        VectorPerElemMinDouble<<<num_blocks, block_size>>>(d_a, d_b, d_result,
                                                           size);
    }
};
