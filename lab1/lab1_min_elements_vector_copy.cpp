...
__global__ void VectorPerElemMinDouble(const double* first_vector,
                                       ...
                                       int32_t start_idx) {
    int32_t idx = start_idx + blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < data_size) {
...
public:
    ...
    [[nodiscard]] std::vector<T> Compute(const std::vector<T>& host_vector_a,
        ...
        constexpr int32_t max_threads = THREADS_PER_BLOCK;
        for (int32_t start_idx = 0; start_idx < vector_size_;
             start_idx += max_threads * max_threads) {
            int32_t remaining = vector_size_ - start_idx;
            int32_t blocks = (remaining + max_threads - 1) / max_threads;

            RunCudaKernel(blocks, max_threads, start_idx);
        }
...
private:
    ...
    void RunCudaKernel(int32_t num_blocks, int32_t block_size,
                       int32_t start_idx) {
        VectorPerElemMinDouble<<<num_blocks, block_size>>>(
            device_vector_a_.Get(), device_vector_b_.Get(),
            device_result_.Get(), vector_size_, start_idx);
    }
};
